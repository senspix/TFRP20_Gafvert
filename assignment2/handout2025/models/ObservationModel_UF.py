...
class ObservationModelUF:
    def __init__(self, stateModel):
        ...
        self.__vectors = np.ones(shape=(self.__num_readings, self.__dim)) # why not zeros? (last row is error reading)

        # is the use of '+=' necessary below?
        for o in range(self.__num_readings - 1): #loop over possible readings (minus error)
            x, y = self.__stateModel.reading_to_position(o) # observed position

            for i in range(self.__dim): # loop over all states
                sx, sy = self.__stateModel.state_to_position(i) # true position
                self.__vectors[o, i] = 0.0 # better initialize all matrix to zero?
    ...
    def get_o_reading_state(self, reading: int, i: int) -> float:
        if reading == None : reading = self.__num_readings-1 # strange. never used?
    ...
    def sum_diags(self):
        dummy = np.diag( self.__vectors[0,:]) # why not zero and loop from 0?
